  ...
  <div class="app-container">
    <div class="container">
      <div class="answer-box" v-if="data.type==0">
        ...
        <div class="questionBox" v-for="(item,index) in data.questionList" :key="index">
         <div class="label" v-if="index==0">{{item.instruction}}</div>
          <div class="label" v-else-if="index<data.questionList.length-1&&item.instruction!=data.questionList[index-1].instruction">{{item.instruction}}</div>
          <div class="question">{{index+1}}、{{item.question.question}}</div>
          <el-radio-group v-model="item.question.answer">
            <div
              class="question"
              v-for="(itemData,indexData) in item.question.answers"
              :key="indexData"
            >
          ...
          <div class="question" v-for="(itemData,indexData) in item.data" :key="indexData">
            <div class="question flex" v-if="itemData.type==='0'">
              ...
              <el-radio-group v-model="itemData.answer" class="flex" disabled>
                <el-radio
                  v-model="itemData.answers"
                  v-for="(itemData2,itemIndex2) in itemData.answers"
                  :label="itemIndex2+1"
                  :key="itemIndex2"
                >{{itemData2}}</el-radio>
              </el-radio-group>
            ...
            <div class="question" v-else-if="itemData.type==='1'">
              <span>({{indexData+1}})、{{itemData.label}}</span>
              <el-input-number v-model="item.answer" size="small" disabled></el-input-number>
            </div>
            <div class="question" v-else-if="itemData.type==='2'">
              <span>({{indexData+1}})、{{itemData.label}}</span>
              <el-input class="remark" v-model="item.answer" disabled></el-input>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="questionBox" v-for="(item,index) in data.problem" :key="index">
          ...
          <div v-if="item.type=='1'" class="symptom">
            <el-checkbox-group v-model="item.data" disabled>
              <el-checkbox
                v-for="(itemData,indexData) in item.symptom"
                :label="itemData"
                :key="indexData"
              >{{itemData.question}}</el-checkbox>
            </el-checkbox-group>
            <div class="symptom" v-for="(itemData,indexData) in item.data" :key="indexData">
              ...
              <el-radio-group v-model="itemData.answer" disabled>
                <el-radio
                  v-for="(itemData2,indexData2) in itemData.answers"
                  :label="indexData2"
                  :key="indexData2"
                >{{itemData2}}</el-radio>
              </el-radio-group>
          ...
          <div v-else-if="item.type=='2'">
            <el-checkbox-group v-model="item.data" disabled v-if="gender">
              <div
                class="question"
                v-for="(itemData,indexData) in item.symptom[1].data"
                :key="indexData"
              >
